...
import ae.rta.dls.backend.repository.EntityAuditEventRepository;
...
public class AsyncEntityAuditEventWriter {
    ...
    public void writeAuditEvent(Object target, EntityAuditAction action) {
        String debugMsg = String.format("-------------- Post %s audit  --------------", action.value());
        log.debug(debugMsg);
        try {
            ...
            log.error(String.format("Exception while persisting audit entity for %s error: %s", target,e));
        }
    }
    ...
    private EntityAuditEvent prepareAuditEntity(final Object entity, EntityAuditAction action) {
        ...
        if (EntityAuditAction.CREATE.equals(action)) {
            ...
            if(StringUtil.isNotBlank(abstractAuditEntity.getCreatedBy())) {
                String[] created = abstractAuditEntity.getCreatedBy().split("#");
                if (created.length == 2) {
                    auditedEntity.setAuthority(created[1]);
                }
            }
            ...
            if(StringUtil.isNotBlank(abstractAuditEntity.getLastModifiedBy())) {
                String[] modified =   abstractAuditEntity.getLastModifiedBy().split("#");
                if(modified.length == 2) {
                   auditedEntity.setAuthority(modified[1]);
                }
            }
            calculateVersion(auditedEntity);
        }

        String traceMsg = String.format("Audit Entity --> %s", auditedEntity.toString());
        log.trace(traceMsg);
