...
public class LoggingAspect {
     ...
     */
    @Pointcut("within(ae.rta.dls.backend.repository..*)"+
        " || within(ae.rta.dls.backend.service..*)"+
        " || within(ae.rta.dls.backend.web.rest..*)")
    ...
    @Profile(EnvironmentConstants.SPRING_PROFILE_DEVELOPMENT)
    ...
    private void logError (JoinPoint joinPoint, Throwable e) {
        try {
            ErrorLogDTO errorLogDTO = errorLogService.save(new ErrorLogDTO(joinPoint, e, httpRequest));
            Optional<ErrorLogDTO> optionalErrorLog = Optional.ofNullable(errorLogDTO);

            if (optionalErrorLog.isPresent()) {
                httpRequest.setAttribute(Constants.SYSTEM_TRACE_ID, Constants.PREFIX_SYSTEM_TRACE_ID + optionalErrorLog.get().getId());
            }
        } catch(Exception t) {
            log.error("Logging error Failed in LoggingAspect.logError");
        }
    }
}
