 ...
 * @author Mena Emiel.
 */
public class ServiceRequestJsonConverter implements AttributeConverter<ServiceDocumentJsonType, String> {

    @Override
    public String convertToDatabaseColumn(ServiceDocumentJsonType jsonAttributeValue) {
        try {
    ...
    @Override
    public ServiceDocumentJsonType convertToEntityAttribute(String jsonColumnValue) {
        try {
            if (StringUtil.isBlank(jsonColumnValue)) {
                return null;
            }
            return CommonUtil.getMapper().readValue(jsonColumnValue, ServiceDocumentJsonType.class);
        } catch (IOException e) {
