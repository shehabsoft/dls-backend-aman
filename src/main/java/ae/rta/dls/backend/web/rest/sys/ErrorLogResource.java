package ae.rta.dls.backend.web.rest.sys;

import ae.rta.dls.backend.security.util.AuthoritiesConstants;
import ae.rta.dls.backend.service.dto.sys.ErrorLogDTO;
import com.codahale.metrics.annotation.Timed;
import ae.rta.dls.backend.service.sys.ErrorLogService;
import ae.rta.dls.backend.web.rest.errors.BadRequestAlertException;
import ae.rta.dls.backend.web.rest.util.HeaderUtil;
import ae.rta.dls.backend.web.rest.util.PaginationUtil;
import io.github.jhipster.web.util.ResponseUtil;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.http.HttpHeaders;
import org.springframework.http.ResponseEntity;
import org.springframework.security.access.prepost.PreAuthorize;
import org.springframework.web.bind.annotation.*;

import javax.validation.Valid;
import java.net.URI;
import java.net.URISyntaxException;

import java.util.List;
import java.util.Optional;

/**
 * REST controller for managing ErrorLog.
 */
@RestController
@RequestMapping("/api/sys")
@PreAuthorize("hasRole(\"" + AuthoritiesConstants.ADMIN + "\")")
public class ErrorLogResource {

    private final Logger log = LoggerFactory.getLogger(ErrorLogResource.class);

    private static final String ENTITY_NAME = "errorLog";

    private final ErrorLogService errorLogService;

    public ErrorLogResource(ErrorLogService errorLogService) {
        this.errorLogService = errorLogService;
    }

    /**
     * POST  /error-logs : Create a new errorLog.
     *
     * @param errorLogDTO the errorLog to create
     * @return the ResponseEntity with status 201 (Created) and with body the new errorLog, or with status 400 (Bad Request) if the errorLog has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/error-logs")
    @Timed
    public ResponseEntity<ErrorLogDTO> createErrorLog(@Valid @RequestBody ErrorLogDTO errorLogDTO) throws URISyntaxException {
        log.debug("REST request to save ErrorLog : {}", errorLogDTO);
        if (errorLogDTO.getId() != null) {
            throw new BadRequestAlertException("A new errorLog cannot already have an ID", ENTITY_NAME, "idexists");
        }
        ErrorLogDTO result = errorLogService.save(errorLogDTO);
        return ResponseEntity.created(new URI("/api/sys/error-logs/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, result.getId().toString()))
            .body(result);
    }

    /**
     * PUT  /error-logs : Updates an existing errorLog.
     *
     * @param errorLogDTO the errorLog to update
     * @return the ResponseEntity with status 200 (OK) and with body the updated errorLog,
     * or with status 400 (Bad Request) if the errorLog is not valid,
     * or with status 500 (Internal Server Error) if the errorLog couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/error-logs")
    @Timed
    public ResponseEntity<ErrorLogDTO> updateErrorLog(@Valid @RequestBody ErrorLogDTO errorLogDTO) throws URISyntaxException {
        log.debug("REST request to update ErrorLog : {}", errorLogDTO);
        if (errorLogDTO.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        ErrorLogDTO result = errorLogService.save(errorLogDTO);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, errorLogDTO.getId().toString()))
            .body(result);
    }

    /**
     * GET  /error-logs : get all the errorLogs.
     *
     * @param pageable the pagination information
     * @return the ResponseEntity with status 200 (OK) and the list of errorLogs in body
     */
    @GetMapping("/error-logs")
    @Timed
    public ResponseEntity<List<ErrorLogDTO>> getAllErrorLogs(Pageable pageable) {
        log.debug("REST request to get a page of ErrorLogs");
        Page<ErrorLogDTO> page = errorLogService.findAll(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/sys/error-logs");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * GET  /error-logs/:id : get the "id" errorLog.
     *
     * @param id the id of the errorLog to retrieve
     * @return the ResponseEntity with status 200 (OK) and with body the errorLog, or with status 404 (Not Found)
     */
    @GetMapping("/error-logs/{id}")
    @Timed
    public ResponseEntity<ErrorLogDTO> getErrorLog(@PathVariable Long id) {
        log.debug("REST request to get ErrorLog : {}", id);
        Optional<ErrorLogDTO> errorLog = errorLogService.findOne(id);
        return ResponseUtil.wrapOrNotFound(errorLog);
    }

    /**
     * DELETE  /error-logs/:id : delete the "id" errorLog.
     *
     * @param id the id of the errorLog to delete
     * @return the ResponseEntity with status 200 (OK)
     */
    @DeleteMapping("/error-logs/{id}")
    @Timed
    public ResponseEntity<Void> deleteErrorLog(@PathVariable Long id) {
        log.debug("REST request to delete ErrorLog : {}", id);
        errorLogService.delete(id);
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, id.toString())).build();
    }
}
